...
\end{lemma}
\begin{proof}
    Since $\PSL$ acts on $\H^2$ via isometries, if the conditions holds for some $p$ then it holds for all $p\in\H^2$, as the distance from $i$ to any other point is bounded. Without loss of generality we can assume $p=i$ in the upper half-plane. Assuming $X_n$ converges projectively to a rank 1 matrix $X$, one checks immediately that $X(p)$ is in the projective class of $x=\text{Im}(X)$ as follows: by acting via conjugation we can suppose the matrices in canonical form, let us distinguish the three cases. If $X_n$ are loxodromic they converge projectively to $X=\begin{bmatrix}
        1 & 0 \\
        0 & 0
    \end{bmatrix} \\$ in the projective chart $i$ is represented with the vector $[1,0]$ hence we have the thesis. If $X_n$ are parabolic, they converge to $X=\begin{bmatrix}
        1 & 0 \\
        0 & 1 \end{bmatrix}$ and the thesis follows analogously. Finally if $X_n$ are elliptic, they converge to $X=\begin{bmatrix} \end{bmatrix}$
