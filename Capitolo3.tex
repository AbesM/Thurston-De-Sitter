...
\section{Geodesic in $\PSL$} We have already seen geodesic in the general Anti-de Sitter space, we would like to specialize here using the model of $\PSL$ and tools from general Lie Group theory. In particular we recall the following (the necessary tool in Lie groups theory are introduced in \cite{bonsanteseppi}): 
...
However we will not be really interested in oriented geodesics, hence we will have identification $L_{\ell,\jmath}=L_{\ell^{\prime},\jmath^{\prime}}$ where with $\ell^{\prime} $ with denote $\ell$ endowed with the opposite orientation.\\
...
Recall the description of the left and right rulings of $\partial\A^{2,1}$ given in \refeq{ruling}, we can conclude that the endpoints of a spacelike geodesic and is dual are mutually connected by lightlike segments in $\partial\A^{2,1}$. \textcolor{red}{Qua andrebbero inserite figure per referenza.}\\
