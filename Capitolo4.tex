...
We have remarked in Theorem \ref{ConformalMetric} that unparametrized lightlike geodesics only depend on the conformal class of the Lorentzian metric, hence we will just refer to lightlike geodesics in $\AS^{2,1},$ even when we are considering it endowed with the hemispherical metric of \refeq{emispherical}. 
...
\subsection{General facts.} We will now state some general facts that will be useful in our classification.
...
Using the above construction, we can build examples of MGH spacetimes having holonomy any $\omega=(\rho,\varrho)$ of this form. 
...
In short, given a pair $(I,II)$, Expression \refeq{64} provides a Lorentzian metric of constant curvature $-1$ on an open set $U$ in $\Sigma\times\R$ containing the slice $\Sigma\times\{0\}$, and thus a germ of immersion of $\Sigma$ into an Anti-de Sitter three-manifold with immersion data $(I,II)$. We summarize what we have accomplished so far: 
...
\subsection{Gauss map and projections} We can finally define our map for spacelike surfaces in $\A^{2,1}$. We have seen in Proposition \ref{352} that the space of timelike geodesics of $\PSL$ is naturally identified with $\H^2\times\H^2$, where the identification maps a geodesic of the form
\[
...
\begin{definition}
   ...
   It follows from the equivariance shown in Proposition \ref{352}, that the Gauss map $G_\sigma$ is natural with respect to the action of the isometry group, namely: 
    ...
    In terms of the map $\text{Fix},$ Equation \refeq{gaussmap} reads as: 
