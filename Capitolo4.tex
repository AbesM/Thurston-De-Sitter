...
\begin{definition}
    Given an achronal domain $X$ in $\AS^{2,1}\cup\partial\AS^{2,1}$, the \textit{invisible domain} of $X$ is the subset, that we will denote by $\Omega(X),$ of $\AS^{2,1}\setminus X$ defined as the set of points which are not connected to $X$ by no causal path.   
\end{definition}
