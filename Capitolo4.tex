...
\begin{proof}
    ...
    \begin{equation}\label{42}
       ...
       \end{equation}
       Hence the points $(x,f^X(x))$ and $(x^{\prime},f^X(x^{\prime} ))$ are joined by a lightlike segment. If $x,x^{\prime}$ are not antipodal points on $\partial\D$, there is a unique hemispherical geodesic $\eta$ in $\overline{\D}$ joining $x$ to $x^{\prime}$, which must pass through $y$ by \refeq{42}, and which we may assume parametrized by arcleght. In this case the geodesic segment joining $(x,f^X(x))$ to $(x^{\prime},f^X(x^{\prime} ))$ take the form $t\mapsto(\eta(t),f^X(x^{\prime})+t)$, so it passes through $(y,f_+^X(y))=(y,f_-^X(y)).$\\
