...
\begin{proof}
    ...
    \begin{equation}\label{42}
       ...
       \end{equation}
       Hence the points $(x,f^X(x))$ and $(x^{\prime},f^X(x^{\prime} ))$ are joined by a lightlike segment. If $x,x^{\prime}$ are not antipodal points on $\partial\D$, there is a unique hemispherical geodesic $\eta$ in $\overline{\D}$ joining $x$ to $x^{\prime}$, which must pass through $y$ by \refeq{42}, and which we may assume parametrized by arclength. In this case the geodesic segment joining $(x,f^X(x))$ to $(x^{\prime},f^X(x^{\prime} ))$ take the form $t\mapsto(\eta(t),f^X(x^{\prime})+t)$, so it passes through $(y,f_+^X(y))=(y,f_-^X(y)).$\\
...
Let's $\Sigma_r$ be an oriented surface of genus $r\geq 2.$ We recall the definition of Fuchsian representation. 
...
\begin{proposition}\label{623}
    ...
    \begin{itemize}
        ...
        \item The space of spacelike immersion of $\widetilde{\Sigma}$ into $A^{2,1}$, equivariant with respect to a representation $\rho: \pi_1\Sigma\to \text{Isom}_0(\A^{2,1})$, up to the action of $\text{Isom}_\frac{0}(\A^{2.1}$ via post-composition. 
    \end{itemize}
