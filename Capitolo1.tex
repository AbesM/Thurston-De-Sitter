...
\begin{proof}
    ...
    Hence the condition that $M$ has maximal isometry group is equivalent to the condition that every element $f$ of $\Isom(M_K)$ descends in the quotient to an isometry of $M$. This is in turn equivalent to the condition that $f\Gamma f^{-1}=\Gamma$ for every $f\in \Isom(M_K)$, which is the same as saying that $\Gamma$ is normal in $\Isom(M_K)$.
\end{proof}
